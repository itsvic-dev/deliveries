...
object DhlGerDeliveryService : DeliveryService {
  ...
  override suspend fun getParcel(trackingId: String, postCode: String?): Parcel {
    ...
    val shipment =
        resp.shipments.firstOrNull() { it.shipmentDetails.shippingHistory != null }
            ?: throw ParcelNonExistentException()
  ...
  private interface API {
    ...
    suspend fun getShipments(
        ...
        @Header("User-Agent")
        userAgent: String =
            "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/150.0.7871.189 Mobile Safari/537.36"
    ): TrackingResponse
  }

  @JsonClass(generateAdapter = true)
  internal data class TrackingResponse(
      @Json(name = "sendungen") val shipments: List<Shipment>,
  )

  @JsonClass(generateAdapter = true)
  internal data class Shipment(
      val id: String,
      @Json(name = "sendungsdetails") val shipmentDetails: ShipmentDetails,
  )

  @JsonClass(generateAdapter = true)
  internal data class ShipmentDetails(
      @Json(name = "sendungsverlauf") val shippingHistory: ShipmentHistory?,
      @Json(name = "istZugestellt") val isDelivered: Boolean,
      @Json(name = "ruecksendung") val returnShipment: Boolean,
      @Json(name = "zielland") val destinationLocation: String,
      val international: Boolean,
  )

  @JsonClass(generateAdapter = true)
  internal data class ShipmentHistory(
      @Json(name = "fortschritt") val progress: Int,
      val status: String,
  ...
  internal data class Event(
      @Json(name = "datum") val date: String,
      val status: String,
      @Json(name = "ruecksendung") val returnShipment: Boolean,
      @Json(name = "ort") val location: String?,
  )
}
